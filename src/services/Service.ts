 ...
 *   usuario (será criado no Componente Cadastro), que será utilizado 
 *   para enviar os dados, que serão persistidos no Banco de dados do 
 *   Backend, com os dados recebidos no Corpo da Resposta da Requisição,
 *   confirmando a persistência dos dados do usuário. A principal alteração
 *   que será feita neste Estado é a inclusão do id, que foi gerado pelo Banco de dados.
 * 
 ...
 * 
 * - setDados: É a função que será utilizada para atualizar o Estado 
 *   usuario (será criado no Componente AuthContext), que foi criado 
 *   na Context (AuthContext), que será utilizado para armazenar os 
 *   dados do usuário autenticado no Backend, recebidos no Corpo da 
 *   Resposta da Requisição Login, confirmando a autenticação do usuário.
 *   A principal alteração que será feita no Estado usuario é a inclusão 
 *   de todos os dados do usuario como id, nome, foto e o token.
 * 
 * Para enviar a Requisição POST, utilizaremos o método api.post() do Axios.
 */
